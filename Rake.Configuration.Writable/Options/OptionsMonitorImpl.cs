...
using Rake.Configuration.Writable.Abstractions;
using Rake.Configuration.Writable.Diagnostics;
    ...
    where T : class, new()
{
    ...
    private LoadedConfiguration LoadConfigurationFromProvider(string instanceName)
    {
        ...
        try
        {
            ...
            var value = options.FormatProvider.LoadWithMigration(options);
    ...
    )
    {
        ...
        try
        {
            ...
            watcher.Changed += (_, args) => OnFileChanged(options.InstanceName, args);
            watcher.Created += (_, args) => OnFileChanged(options.InstanceName, args);
            watcher.Deleted += (_, args) => OnFileChanged(options.InstanceName, args);
            watcher.Renamed += (_, args) => OnFileChanged(options.InstanceName, args);
            watcher.Error += (_, args) => OnWatcherError(options.InstanceName, args);
