...
namespace Rake.Core.YtDlp.Models;
...
public sealed class Format
{
    ...
    /// </summary>
    public bool IsVideo =>
        !string.IsNullOrEmpty(VideoCodec) && VideoCodec != "none" && Resolution != "audio only";

    /// <summary>
    /// Indicates whether this format is audio-only.
    /// </summary>
    public bool IsAudioOnly =>
        Resolution == "audio only" || (VideoCodec == "none" && !string.IsNullOrEmpty(AudioCodec));
    ...
    public static Format FromParsedLine(string line)
    {
        ...
        if (
            idx < tokens.Count
            && (
                tokens[idx].Contains("MiB")
                || tokens[idx].Contains("GiB")
        ...
        if (
            idx < tokens.Count
            && (
                tokens[idx].StartsWith("http")
                || tokens[idx].Contains("m3u8")
                || tokens[idx] == "mhtml"
            )
        )
        {
