...
public sealed partial class YtDlp
{
    ...
    public string Preview(string url)
    {
        ...
        return string.Join((string?)" ", (IEnumerable<string?>)argsList.Select(EscapeArgument));
    }
    ...
    public async Task<List<string>> GetExtractorsAsync(
        CancellationToken ct = default,
    ...
    )
    {
        ...
        {
            _logger.Log(LogLevel.Warning, "Extractors fetch cancelled.");
            return [];
        }
    }
    ...
    public async Task<List<string>> GetAdobePassListAsync(
        CancellationToken ct = default,
        bool tuneProcess = true
    )
    {
        ...
        {
            _logger.Log(LogLevel.Warning, "Adobe Pass list fetch cancelled.");
            return [];
        }
    }
    ...
    )
    {
        ...
        {
            _logger.Log(LogLevel.Error, "Metadata deserialize failed: {Message}", ex.Message);
            return null;
        }
    }
    ...
    /// </returns>
    /// <exception cref="ArgumentException"></exception>
    public async Task<string?> GetMetadataRawAsync(
        string url,
        CancellationToken ct = default,
        bool tuneProcess = true
    ...
    public async Task<Metadata?> GetDeepMetadataAsync(
        string url,
        CancellationToken ct = default,
        bool tuneProcess = true
    )
    {
        ...
        {
            _logger.Log(LogLevel.Error, $"Deep metadata deserialize failed: {ex.Message}");
            return null;
        }
    }
    ...
    public async Task<string?> GetDeepMetadataRawAsync(
        string url,
        CancellationToken ct = default,
        bool tuneProcess = true
    ...
    )
    {
        ...
        return string.IsNullOrWhiteSpace(output) ? [] : ParseFormats(output);
    }
    ...
    /// </returns>
    /// <exception cref="ArgumentException"></exception>
    public async Task<MetadataLight?> GetMetadataLiteAsync(
        string url,
        CancellationToken ct = default,
        bool tuneProcess = true
    )
    {
        ...
        {
            _logger.Log(LogLevel.Warning, "Metadata lite fetch cancelled.");
            return null;
        }
        catch (Exception ex)
        {
            _logger.Log(LogLevel.Error, $"Metadata lite failed: {ex.Message}");
            return null;
        }
    }
    ...
    /// returns <see langword="null"/> if the process fails, returns no data, or is canceled.
    /// </returns>
    /// <exception cref="ArgumentException"></exception>
    public async Task<Dictionary<string, string>?> GetMetadataLiteAsync(
        string url,
    ...
    )
    {
        ...
        var fieldList = fields.ToList() ?? throw new ArgumentNullException(nameof(fields));
        ...
        try
        {
            ...
            if (parts.Length != fieldList.Count)
                return null;
        ...
        {
            _logger.Log(LogLevel.Warning, "Metadata lite (fields) fetch cancelled.");
            return null;
        }
        catch (Exception ex)
        {
            _logger.Log(LogLevel.Error, $"Metadata lite (fields) failed: {ex.Message}");
            return null;
        }
    }
    ...
    /// </returns>
    /// <exception cref="ArgumentException"></exception>
    public async Task<string> GetBestAudioFormatIdAsync(
        string url,
        CancellationToken ct = default,
        bool tuneProcess = true
    )
    {
        ...
        var best = meta
            ?.Formats?.Where(f => f.IsAudio && (f.Abr > 0 || f.Tbr > 0))
    ...
    /// </returns>
    /// <exception cref="ArgumentException"></exception>
    public async Task<string> GetBestVideoFormatIdAsync(
        string url,
        int maxHeight = 1080,
        CancellationToken ct = default,
        bool tuneProcess = true
    )
    {
        var meta = await GetMetadataAsync(url, ct, tuneProcess);
        var best = meta
            ?.Formats?.Where(f => !f.IsAudio && f.Height.HasValue && f.Height <= maxHeight)
    ...
    /// <exception cref="ArgumentException"></exception>
    public async Task<List<SubtitleTrack>> GetSubtitlesAsync(
        string url,
        CancellationToken ct = default,
        bool tuneProcess = true
    )
    {
        ...
        return string.IsNullOrWhiteSpace(output) ? [] : ParseSubtitles(output);
    }
    ...
    )
    {
        ...
        {
            _logger.Log(LogLevel.Warning, "Probe cancelled.");
            return null;
        }
        catch (Exception ex)
        {
            _logger.Log(LogLevel.Error, $"Probe failed: {ex.Message}");
            return null;
        }
    }

    private async Task<string?> GetMetadataInternalAsync(
        string url,
        bool flat,
    ...
    private List<Format> ParseFormats(string result)
    {
        ...
        foreach (var line in lines)
        {
            ...
            if (!inFormatSection || line.Contains("RESOLUTION") || line.StartsWith("---"))
                continue;
            if (string.IsNullOrWhiteSpace(line) || !Regex.IsMatch(line, @"^\S+\s+\S+"))
                break;
            ...
            {
                _logger.Log(LogLevel.Warning, $"Failed parsing format line: {line} → {ex.Message}");
            }
        }
            ...
            if (match.Success)
            {
                subtitles.Add(
                    new SubtitleTrack
                    {
                        ...
                        Formats = [.. match.Groups[3].Value.Split(',').Select(f => f.Trim())],
                    }
                );
            }
        }
