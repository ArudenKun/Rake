...
public sealed partial class YtDlp
{
    ...
    {
        return this.AddFlag("--extract-audio")
    ...
    /// <returns>A new <see cref="Builder.Ytdlp"/> instance.</returns>
    public YtDlp WithConcatPlaylist(string policy = "always") =>
        AddOption("--concat-playlist", policy);
    ...
    /// <returns>A new <see cref="Builder.Ytdlp"/> instance.</returns>
    public YtDlp WithUsePostProcessor(
        PostProcessors postProcessor,
        string? postProcessorArgs = null
    )
    {
        if (!string.IsNullOrWhiteSpace(postProcessorArgs))
            return AddOption(
                "--use-postprocessor",
                $"{postProcessor.ToString().Trim()}:{postProcessorArgs.Trim()}"
            );
