...
public sealed partial class YtDlp
{
    ...
    )
    {
        ...
        {
            _logger.Log(LogLevel.Error, $"Failed to create required folders: {ex.Message}");
        ...
        var arguments = string.Join(
            (string?)" ",
            (IEnumerable<string?>)argsList.Select(EscapeArgument)
        );

        _logger.Log(
            LogLevel.Information,
            "Executing: {YtdlpPath} {Arguments}",
            _ytdlpPath,
            arguments
        );
        ...
        void OnCompleteDownloadHandler(object? s, string msg) =>
            DownloadCompleted?.Invoke(this, msg);
        void OnPostProcessingStartHandler(object? s, string msg) =>
            PostProcessingStarted?.Invoke(this, msg);
        void OnPostProcessingCompleteHandler(object? s, string msg) =>
            PostProcessingCompleted?.Invoke(this, msg);
        ...
        void OnCommandCompletedHandler(object? s, CommandCompletedEventArgs e) =>
        ...
        try
        {
            await runner.ExecuteAsync(
                ...
                onLineReceived: line =>
                {
                    ...
                    {
                        _logger.Log(LogLevel.Error, "Progress parse error: {Message}", ex.Message);
                    }
    ...
    )
    {
        ...
        var tasks = urlList.Select(async url =>
        {
            ...
            {
                _logger.Log(LogLevel.Error, $"Skipping {url}: {ex.Message}");
            }
            finally
            {
                throttler.Release();
