...
public sealed class ProcessRunner
{
    ...
    )
    {
        ...
        try
        {
            ...
            using var registration = ct.CanBeCanceled
                ? ct.Register(() =>
                {
                    ...
                    _logger.Log(
                        ...
                        "Cancellation requested via token -> Safe killing process tree"
                    );
            ...
            { /* SafeKill fired; pipes closing; drain best-effort */
            }
    ...
    private static async Task WriteAuthAsync(Process process, YtdlpAuthentication authentication)
    {
        if (process.HasExited)
            return;
        ...
        try
        {
            ...
            {
                await process
                    .StandardInput.WriteLineAsync(authentication.Username)
            ...
            {
                await process
                    .StandardInput.WriteLineAsync(authentication.Password)
                    .ConfigureAwait(false);
            }
    ...
    private static async Task WriteAdobePassAsync(Process process, AdobePasswordAuthentication auth)
    {
        if (process.HasExited)
            return;
        ...
        try
        {
            await process.StandardInput.WriteLineAsync(auth.Mso).ConfigureAwait(false);

            await process.StandardInput.WriteLineAsync(auth.Username).ConfigureAwait(false);

            await process.StandardInput.WriteLineAsync(auth.Password).ConfigureAwait(false);
