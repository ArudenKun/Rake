namespace Rake.Core.YtDlp.Helpers;
...
{
    private static readonly string FfmpegName = OperatingSystem.IsWindows()
        ? "ffmpeg.exe"
        : "ffmpeg";

    private static readonly string FfprobeName = OperatingSystem.IsWindows()
        ? "ffprobe.exe"
        : "ffprobe";
    ...
    private static void ValidateFolder(string folder)
    {
        ...
        if (!File.Exists(ffmpegPath))
            throw new FileNotFoundException(
                $"FFmpeg folder must contain ffmpeg binary: {ffmpegPath}"
            );
