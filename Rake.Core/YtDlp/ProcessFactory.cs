...
using Rake.Core.YtDlp.Extensions;
...
public sealed class ProcessFactory
{
    ...
    public static void Tune(Process process, ILogger? logger = null)
    {
        ...
        catch (Exception ex)
        {
            logger?.Log(
                ...
                "Process priority tuning skipped: {Message}",
                ex.Message
            );
        }
    }
    ...
    public static void SafeKill(Process process, ILogger? logger = null)
    {
        try
        {
            ...
            process.KillTree();
            logger?.Log(LogLevel.Information, "Process tree killed successfully.");
        }
        catch (Exception ex)
        {
            logger?.Log(LogLevel.Error, "Failed to kill process tree: {Message}", ex.Message);
        }
    }
    ...
    private static void ValidateBinary(string path)
    {
        ...
        if (fileInfo.Length < 1024)
            throw new InvalidOperationException(
                $"Invalid yt-dlp binary detected: {fileInfo.FullName} ({fileInfo.Length} bytes)."
            );
    }
}
