...
public sealed partial class YtDlp
{
    ...
    public YtDlp WithConfigLocations(string path)
    {
        if (string.IsNullOrWhiteSpace(path))
            throw new ArgumentException("Config folder path required");
    ...
    {
        if (string.IsNullOrWhiteSpace(path))
            throw new ArgumentException("plugin folder path required");
    ...
    public YtDlp WithWaitForVideo(TimeSpan? maxWait = null)
    {
        ...
        var waitValue =
            maxWait.HasValue && maxWait.Value.TotalSeconds > 0
    ...
    {
        if (string.IsNullOrWhiteSpace(alias) || string.IsNullOrWhiteSpace(options))
            return this;
    ...
    public YtDlp WithFFmpegAsLiveDownloader(string? extraFfmpegArgs = null) =>
        WithDownloaderArgs("ffmpeg", extraFfmpegArgs);
    ...
    public YtDlp WithMp4PostProcessingPreset() =>
        this.WithRemuxVideo("mp4").WithEmbedMetadata().WithEmbedChapters().WithEmbedThumbnail();

    /// <summary>
    /// Output to mkv with remux and merge.
    /// </summary>
    /// <returns>A new <see cref="Ytdlp"/> instance.</returns>
    public YtDlp WithMkvOutput() => this.WithRemuxVideo("mkv").WithMergeOutputFormat("mkv");
