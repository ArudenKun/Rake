...
public sealed partial class YtDlp
{
    ...
    public YtDlp WithConcurrentFragments(int count = 8) =>
        count > 0 ? new YtDlp(this, concurrentFragments: count) : this;
    ...
    /// <returns>A new <see cref="Builder.Ytdlp"/> instance.</returns>
    public YtDlp WithRetries(int maxRetries) =>
        AddOption("--retries", maxRetries < 0 ? "infinite" : maxRetries.ToString());
    ...
    /// <param name="maxRetries"></param>
    /// <returns>A new <see cref="Builder.Ytdlp"/> instance.</returns>
    public YtDlp WithFileAccessRetries(int maxRetries) =>
        AddOption("--file-access-retries", maxRetries < 0 ? "infinite" : maxRetries.ToString());
    ...
    {
        if (string.IsNullOrWhiteSpace(retrySleepExpression))
            return this;
    ...
    /// <param name="type">Optional retry type ("http", "fragment", "file_access", "extractor").</param>
    /// <returns>A new <see cref="Builder.Ytdlp"/> instance.</returns>
    public YtDlp WithLinearRetrySleep(
        int start,
        int? end = null,
        int? step = null,
        string? type = null
    ...
    /// <param name="type">Optional retry type ("http", "fragment", "file_access", "extractor").</param>
    /// <returns>A new <see cref="Builder.Ytdlp"/> instance.</returns>
    public YtDlp WithExponentialRetrySleep(
        int start,
        int? end = null,
        double? @base = null,
        string? type = null
