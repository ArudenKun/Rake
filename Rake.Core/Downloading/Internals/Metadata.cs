...
{
    internal const string MetadataExtension = ".meta";
    ...
    internal static async ValueTask<Metadata?> ReadLastMetadataAsync(
        Url? url,
        FileInfo outputFilePath,
        FileInfo metadataFilePath,
        long targetToDownloadSize,
    ...
    )
    {
        try
        {
            ...
            await using (
                FileStream metadataStream = metadataFilePath.Open(
                    FileMode.Open,
                    FileAccess.Read,
                    FileShare.ReadWrite
                )
            )
            {
                ...
                var lastMetadata = await JsonSerializer.DeserializeAsync(
                    metadataStream,
                    MetadataJsonContext.Default.Metadata,
                    token
                );
