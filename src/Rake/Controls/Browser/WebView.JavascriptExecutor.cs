...
namespace Rake.Controls.Browser;
...
partial class WebView
{
    ...
    {
        [DataMember(Name = "stack")] public string Stack = string.Empty;
        [DataMember(Name = "name")] public string Name = string.Empty;
        [DataMember(Name = "message")] public string Message = string.Empty;
    }
    ...
    internal class JavascriptExecutor : IDisposable
    {
        ...
        private static Regex StackFrameRegex { get; } = new(
            @"at\s*(?<method>.*?)\s\(?(?<location>[^\s]+):(?<line>\d+):(?<column>\d+)", RegexOptions.Compiled);
        ...
        public void StartFlush(CefFrame frame)
        {
            ...
            lock (FlushTaskCancellationToken)
            {
                ...
                this.frame = frame;
                flushTask = Task.Factory.StartNew(FlushScripts, FlushTaskCancellationToken.Token,
        ...
        {
            scriptsToExecute = scriptsToExecute.ToArray();
        ...
        public async Task<T> EvaluateScript<T>(string script, string? functionName = null, TimeSpan? timeout = null)
        {
            ...
            void Evaluate(string scriptToEvaluate)
            {
                ...
                try
                {
                    var innerEvaluationTask =
                        OwnerWebView.chromium.EvaluateJavaScript<T>(WrapScriptWithErrorHandling(scriptToEvaluate),
                            timeout: timeout);
                ...
                catch (Exception e)
                {
                    ...
                    {
                        evaluationTask.SetException(new JavascriptException(TimeoutExceptionName,
            ...
            if (timeout.HasValue)
            {
                ...
                if (task != evaluationTask.Task)
                {
                    ...
                    {
                        throw new JavascriptException(TimeoutExceptionName,
                            $"Javascript engine is not initialized after {InitializationTimeout.Seconds}s");
                    }
        ...
        private T GetResult<T>(object result)
        {
            ...
            if (result == null)
            {
                ...
                return
                    default(T); // return default T (its safer, because we allow returning null and converting into a default struct value)
            }
        ...
        {
            return "try {" + script + Environment.NewLine +
                   "} catch (e) { throw JSON.stringify({ stack: e.stack, message: e.message, name: e.name }) + '" +
                   InternalException + "' }";
        }
        ...
        {
            return new JavascriptException("Timeout",
                $"More than {timeout.TotalMilliseconds}ms elapsed evaluating: '{functionName}'");
        }

        private static Exception ParseException(Exception exception, IEnumerable<string> evaluatedScriptFunctions)
        {
            var jsErrorJSON = ((exception is AggregateException aggregateException)
                ? aggregateException.InnerExceptions.FirstOrDefault(e => IsInternalException(e.Message))?.Message
                : exception.Message) ?? "";
            ...
            if (!string.IsNullOrEmpty(jsErrorJSON))
            {
                ...
                if (jsError != null)
                {
                    ...
                    var jsStack = jsError.Stack
                        .Substring(Math.Min(jsError.Stack.Length, (jsError.Name + ": " + jsError.Message).Length))
                    ...
                    foreach (var stackFrame in jsStack)
                    {
                        ...
                        {
                            parsedStack.Add(new JavascriptStackFrame(frameParts.Groups["method"].Value,
                                frameParts.Groups["location"].Value, int.Parse(frameParts.Groups["column"].Value),
                                int.Parse(frameParts.Groups["line"].Value)));
                        }
                    }
