...
partial class WebView
{
    public class JavascriptException : Exception
    {
        ...
        public override string StackTrace
        {
            get
            {
                return string.Join(Environment.NewLine,
                    JsStack.Select(FormatStackFrame).Concat(new[] { BaseStackTrace }));
            }
        }
        ...
        private static string FormatStackFrame(JavascriptStackFrame frame)
        {
            ...
            var location = string.IsNullOrEmpty(frame.ScriptNameOrSourceUrl)
                ? ""
                : ($" in {frame.ScriptNameOrSourceUrl}:line {frame.LineNumber} {frame.Column}");
