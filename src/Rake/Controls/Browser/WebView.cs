...
public partial class WebView : Control, IDisposable
{
    ...
    private Dictionary<string, JavascriptExecutor> JsExecutors { get; } = new();

    private CountdownEvent JavascriptPendingCalls { get; } = new(1);
    ...
    private CancellationTokenSource AsyncCancellationTokenSource { get; } = new();
        ...
        ExecuteWithAsyncErrorHandling(() =>
        {
            ...
            lock (JsExecutors)
            {
                ...
                if (this.IsMainFrame(frameName))
                {
                    ...
                    DisposeJavascriptExecutors(
                        JsExecutors.Where(je => !je.Value.IsValid).Select(je => je.Key).ToArray());
                }
