...
public sealed class ConstructorBuilder
    : BuilderBase<ConstructorBuilder>,
        IParameterized<ConstructorBuilder>
{
    private readonly List<ParameterBuilder<ConstructorBuilder>> _parameters =
        new List<ParameterBuilder<ConstructorBuilder>>();
    ...
    List<ParameterBuilder<ConstructorBuilder>> IParameterized<ConstructorBuilder>.Parameters =>
        _parameters;
    ...
    public ConstructorBuilder WithParameterDoc(string paramName, string documentation)
    {
        if (_xmlDoc is not null && _xmlDoc is not ParameterDocumentationComment)
            throw new Exception(
                "Documentation Comment has already been initialized using an InheritDoc."
            );
    ...
    internal override void Write(in CodeWriter writer)
    {
        ...
        var parameters = _parameters.Any()
            ? string.Join(", ", _parameters.Select(x => x.ToString()))
            : string.Empty;
