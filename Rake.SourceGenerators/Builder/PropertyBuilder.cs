...
public class PropertyBuilder : IBuilder
{
    ...
    public ClassBuilder UseAutoProps() => UseAutoProps(null);
    ...
    public ClassBuilder MakeBackingField(string? defaultValue = null) => WithValue(defaultValue);

    public PropertyBuilder WithBackingField(
    ...
    void IBuilder.Write(in CodeWriter writer)
    {
        ...
        var output = (
            ...
            {
                FieldType.Const =>
                    $"{AccessibilityHelpers.Code(AccessModifier)}{isNew} const {type} {name}",
                FieldType.ReadOnly =>
                    $"{AccessibilityHelpers.Code(AccessModifier)}{isNew}{_static} readonly {type} {name}",
        ...
        if (
            !string.IsNullOrEmpty(_getterExpression)
