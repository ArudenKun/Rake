...
public sealed class LogicalConditionBuilder
{
    ...
    internal LogicalConditionBuilder(
        ICodeWriter writer,
        string? condition,
        string @operator = "if",
        LogicalConditionBuilder? parent = null
    )
    {
        _writer = writer;
    ...
    {
        var builder = new LogicalConditionBuilder(_writer, condition, "else if", this).WithBody(
            innerWrite
        );
        _builders.Enqueue(builder);
        return builder;
    ...
    {
        var builder = new LogicalConditionBuilder(_writer, null, "else", this).WithBody(innerWrite);
        _builders.Enqueue(builder);
        return builder;
    }
    ...
    {
        var expression = string.IsNullOrEmpty(_condition)
            ? _operator
            : $"{_operator} ({_condition})";
