...
namespace Rake.SourceGenerators.Builder.Extensions;
...
public static class CodeHelpersGeneratorExtensions
{
    ...
    public static void AddSource(this GeneratorExecutionContext context, CodeBuilder builder)
    {
        ...
        if (builder.Classes.Count == 1 && builder.Enums.Count == 0 && builder.Records.Count == 0)
        {
            ...
            builder.Classes.Count == 0
            && builder.Enums.Count == 1
            && builder.Records.Count == 0
        )
        {
            ...
            builder.Classes.Count == 0
            && builder.Enums.Count == 0
            && builder.Records.Count == 1
        )
        {
        ...
        {
            context.AddSource(CodeBuilder.Create(builder.Namespace).AddClass(@class));
        }

        foreach (var @enum in builder.Enums)
        {
            context.AddSource(CodeBuilder.Create(builder.Namespace).AddEnum(@enum));
        }

        foreach (var @record in builder.Records)
        {
            context.AddSource(CodeBuilder.Create(builder.Namespace).AddRecord(@record));
        }
    }
    ...
    private static SourceText SourceText(CodeBuilder builder, ParseOptions? options)
    {
        if (options is CSharpParseOptions parseOptions)
        {
            ...
            return Microsoft.CodeAnalysis.Text.SourceText.From(
                CSharpSyntaxTree.Create(formattedRoot).ToString(),
                Encoding.UTF8
            );
        }
