...
public static class ParameterizedExtensions
{
    ...
    {
        return parameterized.AddParameterInternal(
            GetParameter(parameterized.Parent, typeName, null),
            -1
        );
    }
    ...
    public static T AddParameter<T>(
        this IParameterized<T> parameterized,
        string typeName,
    ...
    {
        return parameterized.AddParameterInternal(
            GetParameter(parameterized.Parent, typeName, parameterName),
            -1
        );
    }
    ...
    {
        parameterized.Parent.AddNamespaceImport(typeSymbol);
        return parameterized.AddParameter(
            SymbolHelpers.GetGloballyQualifiedTypeName(typeSymbol),
            parameterName
        );
    }
    ...
    {
        parameterized.Parent.AddNamespaceImport(typeSymbol);
        return parameterized.AddParameterWithDefaultValue(
            typeSymbol.Name,
            parameterName,
            defaultValue,
            index
    ...
    public static T AddParameterWithNullValue<T>(
        this IParameterized<T> parameterized,
        string typeName,
        string? parameterName = null,
        int index = -1
    ...
    public static T AddParameters<T>(
        this IParameterized<T> parameterized,
    ...
    private static T AddParameterInternal<T>(
        this IParameterized<T> parameterized,
        ParameterBuilder<T> parameter,
        int index
    )
        where T : BuilderBase<T>, IParameterized<T>
    {
    ...
    private static ParameterBuilder<T> GetParameter<T>(
        T parent,
        string typeName,
        string? parameterName
    )
        where T : BuilderBase<T>
    {
        ...
        return new ParameterBuilder<T>(parent) { Type = typeName, Name = parameterName };
    }
}
