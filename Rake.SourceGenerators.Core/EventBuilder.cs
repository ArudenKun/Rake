...
public class EventBuilder : BuilderBase<EventBuilder>
{
    ...
    public ClassBuilder WithDefaultExplicitImplementation(
        ...
        Accessibility backingFieldAccessibility = Accessibility.Private,
    ...
    internal override void Write(in CodeWriter writer)
    {
        ...
        var eventDeclaration =
            $"{AccessibilityHelpers.Code(_declaredAccessibility)} {@static}event {_eventDelegateType} {Name}";
        if (!string.IsNullOrEmpty(_explicitImplementation))
        {
            eventDeclaration =
                $"event {@static}{_eventDelegateType} {_explicitImplementation}.{Name}";
        }
