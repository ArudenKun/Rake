...
internal class ChunkSession
{
    ...
    )
    {
        ...
        long contentLength = await url.GetUrlContentLengthAsync(
            client,
            retryMaxAttempt,
        ...
        downloaderProgress.SetBytesTotal(contentLength);
        ...
        Metadata? currentSessionMetadata = await Metadata.ReadLastMetadataAsync(
            url,
            outputFileInfo,
            metadataFileInfo,
            contentLength,
        ...
        {
            downloaderProgress.AdvanceBytesDownloaded(contentLength);
            progressDelegateAsync?.Invoke(0, downloaderProgress);
            yield break;
        }
        ...
        )
        {
            ...
            downloaderProgress.AdvanceBytesDownloaded(outputFileInfo.Length);
            progressDelegateAsync?.Invoke(0, downloaderProgress);
        }
        ...
        else if (outputFileInfo.Exists)
        {
            ...
            List<ChunkRange?> copyOfExistingRanges = new List<ChunkRange?>(
                currentSessionMetadata.Ranges
            );
            foreach (ChunkRange? range in copyOfExistingRanges)
            {
                ...
                downloaderProgress.AdvanceBytesDownloaded(toAdd);
                progressDelegateAsync?.Invoke(0, downloaderProgress);
        ...
        while (remainedSize > 0)
        {
            ...
            ChunkSession chunkSession = new ChunkSession
            {
                CurrentHttpClient = client,
                CurrentMetadata = currentSessionMetadata,
                CurrentPositions = new ChunkRange { Start = startOffset, End = endOffset },
                RetryAttemptInterval = retryAttemptInterval,
                RetryMaxAttempt = retryMaxAttempt,
                TimeoutAfterInterval = timeoutInterval,
            };
    ...
    private static unsafe void CheckInvalidZeroDataAtStart(
        ChunkRange range,
        FileInfo existingFileInfo,
        List<ChunkRange?> listOfRanges
    )
    {
        ...
        try
        {
            ...
            if (range.Start > nearbyEnd)
            {
                // Get the da stream
                using (
                    FileStream fileStream = existingFileInfo.Open(
    ...
    )
    {
        ...
        HttpResponseInputStream? networkStream = await HttpResponseInputStream.CreateStreamAsync(
            client,
            url,
            offsetStart,
            offsetEnd,
